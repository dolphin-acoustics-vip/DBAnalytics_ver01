	...
	*/    
	public static Double getWTDev(Connection conn, int dbLength) throws SQLException {
		...
		try {
	        ...
	        sample_variance = results.getDouble(1);
		...
		Double standard_deviation = Math.sqrt(sample_variance); // Approx
          ...
          for (int s=0; s<blobSizes.length; s++) {
              ...
              for (int t=0; t<blobTypes.length; t++) {
		            ...
		            String url = "jdbc:sqlite:" + fileName + ".db";   
		    	  	...
		    	  	String filePath = fileName +".csv"; //"\\src\\..\\" + 
...
//		    		}
			      	...
			      	try (PrintWriter writer = new PrintWriter(filePath)) {
			          	...
			          	sb.append("Run , DB Rows , Blob Size (Bytes) , Blob Type , DB Size (Bytes) , Total Write Time , "
			          		...
			          		+ "Most Common Write Time (s) , Write Time Increase\n");
			          	writer.write(sb.toString());
			            
				      	sb = new StringBuilder();
				
//				      	int runs = 100; // Run 100 Times
